...
## 2026-09-05: Explicit unauthenticated trusted-LAN mode

Keep coordinator authentication enabled by default, but permit the deployment to set `BackBurner:RequireAuthentication=false` while port 5080 is reachable only from the household LAN. In that mode both administrator and worker APIs accept requests without keys, workers omit an empty authentication header, and the browser discovers the effective mode from a public configuration endpoint and hides the key prompt. Treat network isolation as the security boundary: authentication must be enabled before routing, proxying, or exposing the coordinator to any untrusted network.
